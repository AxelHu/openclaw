...
// --- Retryable Feishu API error handling ---
// Feishu rate-limits bot message sending (5 QPS per chat, 100/min per bot).
// Transient overload errors should be retried with backoff rather than dropped.
...
async function withFeishuRetry<T>(
  ...
  for (let attempt = 0; attempt < SEND_MAX_RETRIES; attempt++) {
    try {
      ...
      log?.(`feishu: retryable error (code=${extractFeishuErrorCode(err)}), retrying in ${delay}ms (attempt ${attempt + 1}/${SEND_MAX_RETRIES})`);
      await sleep(delay);
    }
  }
